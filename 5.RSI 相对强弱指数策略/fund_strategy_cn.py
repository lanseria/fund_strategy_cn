...
# --- RSI策略参数 ---
RSI_PERIOD = 14                   # RSI的计算周期 (通常为14)
RSI_UPPER = 70.0                  # RSI超买阈值
RSI_LOWER = 30.0                  # RSI超卖阈值
...
class RsiStrategy(bt.Strategy):
    ...
    def __init__(self):
        # 使用 backtrader 内置的RSI指标
        self.rsi = bt.indicators.RelativeStrengthIndex(
            period=self.p.period,
            upperband=self.p.upper, # 这两个参数主要用于绘图
            lowerband=self.p.lower
        )
    ...
    def next(self):
        ...
        if not self.position:
            ...
            if self.rsi[0] <= self.p.lower:
                print(f"\n{self.data.datetime.date()}: RSI ({self.rsi[0]:.2f}) 进入超卖区 (<= {self.p.lower})，准备买入。")
                # 使用 Sizer 全仓买入
                self.order = self.buy()
        # 如果当前持有仓位
        else:
            ...
            if self.rsi[0] >= self.p.upper:
                print(f"\n{self.data.datetime.date()}: RSI ({self.rsi[0]:.2f}) 进入超买区 (>= {self.p.upper})，准备卖出。")
                # 卖出全部仓位
    ...
    def stop(self):
        ...
        report_filename = f'strategy_report_{FUND_SYMBOL}_rsi.html'
        try:
            qs.reports.html(returns, benchmark=benchmark_rets, output=report_filename, 
                            title=f'{FUND_SYMBOL} - RSI相对强弱指数策略 vs. 基准指数')
